...
describe(`"computeResult" function test`, () => {
  ...
  it(`Testing minimal amount of correct answers`, () => {
    assert.equal(computeResult(createAnswersArray(RESULT_CONFIGS.CORRECTS_TO_WIN - 1, false, 10), RESULT_CONFIGS.MAX_LIVES), RESULT_CONFIGS.POINTS_WHEN_LOST, `should return correct amount of points when there is too few correct answers`);
  ...
  it(`Testing point computation`, () => {
    ...
    assert.equal(computeResult(createAnswersArray(RESULT_CONFIGS.CORRECTS_TO_WIN, true, RESULT_CONFIGS.TIME_ANSWER_FAST - 1), RESULT_CONFIGS.MAX_LIVES), nPointsAllFast + nPointsForMaxLives, `should return correct amount of points for ${RESULT_CONFIGS.CORRECTS_TO_WIN} fast answers with ${RESULT_CONFIGS.MAX_LIVES} lives`);
    assert.equal(computeResult(createAnswersArray(RESULT_CONFIGS.CORRECTS_TO_WIN, true, RESULT_CONFIGS.TIME_ANSWER_FAST + 1), RESULT_CONFIGS.MAX_LIVES), nPointsAllNormal + nPointsForMaxLives, `should return correct amount of points for ${RESULT_CONFIGS.CORRECTS_TO_WIN} normal answers with ${RESULT_CONFIGS.MAX_LIVES} lives`);
    assert.equal(computeResult(createAnswersArray(RESULT_CONFIGS.CORRECTS_TO_WIN, true, RESULT_CONFIGS.TIME_ANSWER_SLOW + 1), RESULT_CONFIGS.MAX_LIVES), nPointsAllSlow + nPointsForMaxLives, `should return correct amount of points for ${RESULT_CONFIGS.CORRECTS_TO_WIN} slow answers with ${RESULT_CONFIGS.MAX_LIVES} lives`);
